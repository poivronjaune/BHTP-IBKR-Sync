...
for pos in positions:
    ...
    if position_size != 0:
        if position_size > 0:
            ...
            lmt_price = market_price * 0.99     # Slightly below market to increase chance of execution required for after market trades
            order = LimitOrder(action, order_size, lmt_price)
            order.outsideRth = True  # Allow execution outside regular trading hours
            order.exchange = 'ARCA'  # Faster execution for after hours trading
